...
import { CameraController, InputEvent } from "@speckle/viewer";
...
import { Extension, IViewer, GeometryType, MeshBatch } from "@speckle/viewer";
...
export class BoxSelection extends Extension {
  get inject() {
    return [CameraController, SelectionExtension];
  }
  ...
  private idsToSelect: Array<string> | null = null;
  ...
  private onPointerUp() {
    ...
    this.dragBoxMaterial.uniforms.transform.value = new Matrix4().makeScale(
      0,
      0,
      0
    );
  ...
  private getSelectionIds(selectionBox: Box3) {
    ...
    // Đoạn "|| new Matrix4()" thêm vào chỉ để né lỗi TS
    clipMatrix.multiplyMatrices(
      this.viewer.getRenderer().renderingCamera?.projectionMatrix || new Matrix4(),
      this.viewer.getRenderer().renderingCamera?.matrixWorldInverse || new Matrix4()
    );
